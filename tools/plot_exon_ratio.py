...
def main():
    ...
    missing = [g for g in want if g not in set(bin_gene(b[3]) for b in sel)]
    if missing:
        sys.stderr.write("[warn] no bins for: %s\n" % ", ".join(missing))

    xs = list(range(len(sel)))
    ys = [max(-args.ymax, min(args.ymax, b[4])) for b in sel]
    ...
    title = "%s%s: %s" % ((args.sample + "  ") if args.sample else "", ",".join(chroms), " ".join(g for g in want if g not in missing))
    ax.set_title(title, fontsize=10)
    if have_w:
        ax.text(0.995, 0.02, "point size = bin weight; hollow = weight < %.2f" % args.weight_floor,
                transform=ax.transAxes, ha="right", va="bottom", fontsize=6, color="0.4")
    fig.tight_layout()
    fig.savefig(args.out, dpi=150)
    print("[ok] %s: %d bins, %d genes -> %s" % (args.sample or "sample", len(sel), len(want) - len(missing), args.out))
