...
CHANGELOG
  2026-05-22  Added retry-with-exponential-backoff to check_vv_connection().
              ...
              Backward compatible: default behavior matches prior single-attempt
              check when network is healthy; only changes are on failure paths.
"""
...
# Default total budget with 3 attempts: ~90s of waiting + 3 x 30s timeouts.
...
def check_vv_connection(base_url,
    ...
    commonly a gunicorn restart inside the Docker container. After such a
    restart the worker process takes 60-120 seconds to warm up (HGVS
    library load, VVTA / SeqRepo / VDB connection setup, internal cache
    population), so the default backoff schedule (30s, 60s, 120s)
    accommodates one full warm-up cycle.

    Parameters
    ----------
    base_url : str
        Base URL of the VV REST endpoint, e.g. "http://localhost:5001".
    max_attempts : int
        Total number of connection attempts. Default 3.
    initial_backoff : int
        Seconds to wait after the first failure; doubled on each subsequent
        failure (default 30 -> 30, 60, 120s).

    Returns
    -------
    bool
        True on first successful connection (regardless of attempt number).
        False after all max_attempts have been exhausted.
    """
...
def main():
    ...
    # Check VV connection (with retry on transient failures)
